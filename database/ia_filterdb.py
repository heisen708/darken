...
MONGODB_SIZE_LIMIT = 440 * 1024 * 1024  # 440 MB threshold
...
async def check_db_size(db):
    try:
        ...
        # Use storageSize instead of dataSize (Atlas checks this)
...
async def siletxbotz_fetch_media(limit: int) -> List[dict]:
    try:
        if MULTIPLE_DB:
            db_size = await check_db_size(db)  # check primary DB size
